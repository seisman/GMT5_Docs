...
#. ``-R<code><x0>/<y0>/<nx>/<ny>``
   ...
   该方法可以在创建网格文件时使用。 ``<code>`` 是 ``L|C|R`` 和 ``T|M|B`` 两个字符的任意组合，分别表示矩形网格区域的不同位置，比如 ``BL`` 表示左下角，详情参考 :doc:`justfication` 一节。 ``<code>`` 所指定的位置的坐标为 ``(x0, y0)`` ， ``<nx>`` 、 ``<ny>`` 是X和Y方向的网格数，通过 ``-I`` 选项指定的网格间隔即可计算得到区域范围。
